...
/**
 * Debug script for Windows-compatible OSINT email discovery module
 */

const { discoverProjectEmails } = require('./src/osint/emailDiscovery-windows');
const { createLogger } = require('./src/utils/logger');

const logger = createLogger('OSINTWindowsDebug');
...
async function testOsintModule() {
  ...
  logger.info('Testing Windows-compatible OSINT email discovery with sample project:');
  ...
  try {
    ...
    if (result.alternate_emails && result.alternate_emails.length > 0) {
      ...
      result.alternate_emails.forEach((email, index) => {
    ...
    Object.entries(result.evidence).forEach(([source, value]) => {
      if (value) {
